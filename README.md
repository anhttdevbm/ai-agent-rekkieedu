...
- **Đề bài:** file `.docx` hoặc dán URL Google Docs (`https://docs.google.com/document/d/.../edit`). Tài liệu cần đặt quyền **Bất kỳ ai có liên kết — Xem** (Anyone with the link — Viewer). **Bài nộp:** ô nhiều dòng — **mỗi dòng một** thư mục hoặc link GitHub (cùng một đề, chấm lần lượt); **có thể để trống** dòng đó nếu cùng dòng bên «Repo báo cáo» có link (chỉ chấm báo cáo). Nút **Thêm thư mục…** chèn thêm một dòng. **Repo báo cáo + mini (tuỳ chọn):** ô nhiều dòng — **dòng i khớp bài nộp dòng i**. **Kết quả:** điểm + nhận xét + mini project (có/không) trong log. Muốn JSON dùng CLI `--submission-list` và `--out`.
